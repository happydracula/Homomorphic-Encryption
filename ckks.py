...
class PrivateKey:
    ...
    def decrypt(self, ciphertext):
        pt = utils.mod(ciphertext.ct0 + ciphertext.ct1 *
                       self.sk, self.params.Q, self.params.POLY_MOD)
        # print(pt)
        return Plaintext(utils.small_mod(pt, self.params.Q), ciphertext.scale)
...
class CipherText:
    ...
    def __plain_multiply(self, pt):
        ...
        res1 = utils.small_mod(res1, self.modulus)
        return CipherText(res0, res1, self.sw0, self.sw1, self.scale*pt.scale, self.modulus, self.params)
